...
public class AndroidKeyStoreFacade {
    ...
    private synchronized void initialize() throws KeyStoreException, CryptoException {
        if (keyStore != null) {
            return;
        }
        try {
            ...
            throw new CryptoException("Keystore could not be initialized", e);
        }
    }


    public byte[] encryptKey(byte[] sessionKey) throws KeyStoreException, CryptoException {
        initialize();
        ...
        if (keyStore.containsAlias(ASYMMETRIC_KEY_ALIAS)) {
            ...
            try {
                ...
                throw new CryptoException("encryptKey", e);
            }
        } else {
            Key key = getSymmetricKey();
            return NotificationUtil.encryptKeyWithJavaKey(sessionKey, key);
        }
    }

    public byte[] decryptKey(byte[] encSessionKey) throws UnrecoverableEntryException, KeyStoreException, CryptoException {
        initialize();
        ...
        if (keyStore.containsAlias(ASYMMETRIC_KEY_ALIAS)) {
            ...
            try {
                ...
                throw new CryptoException("decryptKey", e);
            }
        } else {
            Key key = getSymmetricKey();
            return NotificationUtil.decryptKeyWithJavaKey(encSessionKey, key);
        }
    }
    ...
    private Cipher createRSACipher(Key key, int mode) throws CryptoException {
        ...
        try {
            ...
            throw new CryptoException("createRSAcypther", e);
        }
    }
}
